...
function BarChartCustom({ data }: { data: HabitStat[] }) {
  ...
  return (
    <View style={bc.wrap}>
      ...
      {[1, 0.5, 0].map(frac => (
        <View
          key={frac}
          style={[bc.gridLine, { bottom: frac * maxH + 28 }]}
        />
      ))}
      {/* Bars */}
      <View style={bc.barsRow}>
        {data.map((item, i) => {
          ...
          return (
            <View key={i} style={[bc.barCol, { width: barWidth }]}>
              <Text style={bc.barValue}>{item.total}</Text>
              <View
                style={[
                  bc.bar,
                  {
              ...
              />
              <Text style={bc.barLabel} numberOfLines={1}>
...
function DonutRing({
  ...
  // Build arc segments — approximate using the two-half-circle clip trick
  // For simplicity with multiple segments: use stacked rings per category
  // ordered by value desc, innermost = smallest
  ...
  return (
    <View style={{ alignItems: 'center', gap: Spacing.lg, width: '100%' }}>
      {/* Stacked rings — one per category */}
      <View style={{ width: size, height: size, position: 'relative' }}>
        {sorted.map((cat, i) => {
          ...
          return (
            ...
            >
              ...
              {/* Right fill */}
              <View style={{
                position: 'absolute', left: s / 2, top: 0,
                width: s / 2, height: s, overflow: 'hidden',
              }}>
                <View style={{
                  position: 'absolute', left: -s / 2, top: 0,
                  width: s, height: s,
                  borderRadius: r + bw / 2,
                  borderWidth: bw, borderColor: cat.color,
        ...
        <View style={{
          ...
          <Text style={{ color: Colors.textSecondary, fontSize: 11 }}>logs</Text>
        </View>
      </View>
      ...
      <View style={{ gap: 8, width: '100%' }}>
        {sorted.map((cat, i) => (
          <View key={i} style={{ alignItems: 'center', flexDirection: 'row', gap: 10 }}>
            ...
            <Text style={{ color: Colors.textTertiary, fontSize: 11, width: 36, textAlign: 'right' }}>
...
// ─── Heatmap (replaces line chart) ───────────────────────────────────────────

function ActivityHeatmap({ days, dayStats }: { days: number; dayStats: DayStat[] }) {
  ...
  // Pad start so first day aligns to correct column (Mon=0)
  const firstDate  = new Date(allDays[0]);
  const firstDow   = (firstDate.getDay() + 6) % 7; // Mon-based
      ...
      <View style={{ flexDirection: 'row', gap: cellGap, marginBottom: cellGap, paddingLeft: 2 }}>
        {dayLabels.map((d, i) => (
          <View key={i} style={{ width: cellSize, alignItems: 'center' }}>
            <Text style={{ color: Colors.textTertiary, fontSize: 9 }}>{d}</Text>
          </View>
        ))}
      </View>
          ...
          <View key={rowIdx} style={{ flexDirection: 'row', gap: cellGap }}>
            {Array.from({ length: cols }).map((_, colIdx) => {
              ...
              return (
                ...
                >
                  {days <= 7 && ymd ? (
                    <Text style={{ color: val > 0 ? Colors.textPrimary : Colors.textTertiary, fontSize: 11, fontWeight: '700' }}>
                      {val > 0 ? val : ''}
                    </Text>
                  ) : null}
                </View>
      ...
      <View style={{ alignItems: 'center', flexDirection: 'row', gap: 4, marginTop: 10 }}>
        ...
        {[0.1, 0.3, 0.55, 0.8, 1].map((a, i) => (
          <View
            key={i}
            style={{
              backgroundColor: Colors.primary + Math.round(a * 255).toString(16).padStart(2, '0'),
              borderRadius: 2,
              height: 10,
              width: 10,
            }}
    ...
    try {
      ...
      const hStats: HabitStat[] = habitRows
        .map(h => ({
          ...
          total:         Math.round(habitLogMap.get(h.id) ?? 0),
      ...
      const dStats: DayStat[] = Array.from(dayMap.entries()).map(([ymd, total]) => ({ ymd, total }));
      setDayStats(dStats);
      ...
      >
        ...
        {!loading && !hasData && (
          <View style={styles.emptyState}>
            <Ionicons name="bar-chart-outline" size={52} color={Colors.textTertiary} />
        ...
        {!loading && hasData && (
          <>
            ...
            <View style={styles.summaryRow}>
              <View style={[styles.summaryCard, { borderColor: Colors.primaryLight + '33' }]}>
                <View style={[styles.summaryIcon, { backgroundColor: Colors.primaryLight + '18' }]}>
              ...
              <View style={[styles.summaryCard, { borderColor: Colors.accent + '33' }]}>
              ...
              <View style={[styles.summaryCard, { borderColor: Colors.warning + '33' }]}>
            ...
            {habitStats.length > 0 && (
              <View style={styles.chartCard}>
                ...
                <BarChartCustom data={habitStats} />
              </View>
            )}
            ...
            {dayStats.length > 0 && (
              <View style={styles.chartCard}>
                ...
                <Text style={styles.chartSub}>
                  Darker = more logs · today outlined
                </Text>
                <ActivityHeatmap days={days} dayStats={dayStats} />
              </View>
            )}
            ...
            {catStats.length > 0 && (
              <View style={styles.chartCard}>
                ...
                <DonutRing data={catStats} total={catTotal} />
              </View>
            )}
          </>
        )}
